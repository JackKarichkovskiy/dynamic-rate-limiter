...
@SpringBootTest(properties = {"app.rate-limiter.token-bucket.rpm=2"})
@Testcontainers
class TokenBucketRateLimiterTest {
    ...
    void validateRequest_firstRequest_allowed() {
        ...
        // when
        Mono<RateLimitingResult> resultMono = tokenBucketRateLimiter.validateRequest(requestContext);
    ...
    @Test
    void validateRequest_rpmReachedButOnMinuteEdges_forbidden() {
        // given
        var requestContext = CustomRequestContext.builder()
                .userId("validateRequest_rpmReachedButOnMinuteEdges_forbidden").build();
    ...
    void validateRequest_differentUserId_allowed() {
        ...
        for (int i = 0; i < 2; i++) {
            // when
            Mono<RateLimitingResult> resultMono = tokenBucketRateLimiter.validateRequest(requestContext1);
        ...
        for (int i = 0; i < 2; i++) {
            // when
            Mono<RateLimitingResult> resultMono = tokenBucketRateLimiter.validateRequest(requestContext2);
